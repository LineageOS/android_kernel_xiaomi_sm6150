				     ...
				     unsigned long event, void *data)
{
	...
	if (evdata && evdata->data) {
		...
		case MSM_DRM_BLANK_POWERDOWN:
			screen_on = false;
			if (time_after(jiffies, compaction_forced_timeout) &&
			    !delayed_work_busy(&compaction_work)) {
				compaction_forced_timeout = jiffies +
					msecs_to_jiffies(compaction_timeout_ms);
