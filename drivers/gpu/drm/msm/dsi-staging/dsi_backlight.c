...
static int dsi_backlight_update_status(struct backlight_device *bd, void *display)
{
	...
	if (dsi_panel_initialized(panel)) {
        if (drm_dev &&
            ...
            rc = dsi_panel_set_doze_backlight(display, bl_lvl);
            if (rc) pr_err("unable to set doze backlight\n");
            rc = dsi_panel_enable_doze_backlight(panel, bl_lvl);
            if (rc) pr_err("unable to enable doze backlight\n");
        } else {
            rc = dsi_panel_set_backlight(panel, bl_lvl);
            if (rc) pr_err("unable to set backlight (%d)\n", rc);
            goto done;
        }
    }
    bl->bl_actual = bl_lvl;
