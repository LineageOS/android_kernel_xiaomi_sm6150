...
static int interpolate(int x, int xa, int xb, int ya, int yb) {
    ...
    factor = bf / 2;
    plus = bf % 2;
    if ((xa - xb) && (yb - ya)) sub = 2 * (x - xa) * (x - xb) / (yb - ya) / (xa - xb);
...
static int brightness_to_alpha(u32 brightness) {
    ...
    for (index = 0; index < ARRAY_SIZE(brightness_alpha_lut); index++) {
        if (brightness_alpha_lut[index][BRIGHTNESS] >= brightness) break;
    }
    ...
    if (index == 0)
        ...
        alpha = interpolate(brightness, brightness_alpha_lut[index - 1][BRIGHTNESS],
...
void fod_backlight_workaround(void) {
    ...
    schedule_delayed_work(&restore_backlight_work, msecs_to_jiffies(200));
}
