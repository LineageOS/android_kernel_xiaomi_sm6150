...
static int sync_fill_fence_info(struct dma_fence *fence, void __user *uinfo)
{
	struct {
		__s32	status;
		__u32	flags;
				       ...
				       unsigned long arg)
{
	...
	for (i = 0; i < num_fences; i++) {
		void __user *finfo = u64_to_user_ptr(info.sync_fence_info) +
				     i * sizeof(struct sync_fence_info) + offset;
