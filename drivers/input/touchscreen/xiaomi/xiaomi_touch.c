...
struct device_attribute *attr, char *buf)
{
	...
	if (touch_data->getModeValue) {
		ret = touch_data->getModeValue(Touch_Doubletap_Mode, SET_CUR_VALUE);
	} else {
		MI_TOUCH_LOGE(1, "%s %s: getModeValue not implemented", MI_TAG, __func__);
	}
...
struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (touch_data->setModeValue) {
		...
		MI_TOUCH_LOGI(1, "%s %s: set double tap to %d", MI_TAG, __func__, input);
	} else {
