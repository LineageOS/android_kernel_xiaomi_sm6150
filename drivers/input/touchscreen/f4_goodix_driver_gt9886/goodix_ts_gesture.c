			   ...
			   struct goodix_ext_module *module)
{
	...
	ts_info("enter %s temp_data[0]=%d ret=%d\n", __func__, temp_data[0], ret);
	...
	ts_debug("fod enabled: %d, aod status: %d, temp data[2]: %d, sleep finger: %d",
		core_data->fod_enabled, core_data->aod_status, temp_data[2], core_data->sleep_finger);
	if (core_data->fod_enabled || core_data->aod_status) {
		if ((FP_Event_Gesture == 1) && (temp_data[2] == 0x46) && !core_data->sleep_finger) {
			...
			ts_debug("Gesture report, x=%d, y=%d, overlapping_area=%d, area=%d", x, y, overlapping_area, area);
